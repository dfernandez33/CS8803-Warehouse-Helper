...
class GUIWindow():
    ...
    def _show_robot(self, robot):
        ...
        self.colorLine(coord, (coord[0]+fov_lx, coord[1]+fov_ly), color='#222222', line_width=2, dashed=True)
        self.colorLine(coord, (coord[0]+fov_rx, coord[1]+fov_ry), color='#222222', line_width=2, dashed=True)
    ...
    def colorCell(self, location, color):
        ...
        self.canvas.create_rectangle(coords[0], coords[1], coords[0] + self.grid.scale, coords[1] + self.grid.scale,
                                     fill=color)
    ...
    def colorLine(self, coord1, coord2, color='black', line_width=1, dashed=False):
        if dashed:
            self.canvas.create_line(coord1[0] * self.grid.scale, (self.height-coord1[1]) * self.grid.scale,
                                    ...
                                    fill=color, width=line_width, dash=(5, 3))
        else:
            self.canvas.create_line(coord1[0] * self.grid.scale, (self.height-coord1[1]) * self.grid.scale,
                                    coord2[0] * self.grid.scale, (self.height-coord2[1]) * self.grid.scale,
    ...
    def start(self):
        ...
        if self.show_camera:
            self.camera_canvas = Canvas(master, width=self.grid.width*self.grid.scale,
                                        height=self.grid.height*self.grid.scale, bd=0, bg='#ffffff')
